...
namespace RoboCup.Entities
{
    public class Coach : ICoach
    {
		...
		public List<SeenCoachObject> GetAlllayers(int i)
		{
			List<SeenCoachObject> palyers = new List<SeenCoachObject>();
			var dic = GetSeenCoachObjects();
			var teamName = SoccerParams.m_teamName;
			var myName = $"player {teamName} {i}";
			var gk = $"player {teamName} 5";
			var startTeamName = $"player";

			foreach (var key in dic.Keys)
			{
				if (key.StartsWith(startTeamName) && key != myName && key != gk)
				{
					palyers.Add(dic[key]);
				}
			}
		...
		public SeenCoachObject GetPlayer(string name)
        {
            var dic = GetSeenCoachObjects();
            foreach (var key in dic.Keys)
            {
        ...
        public List<SeenCoachObject> GetMyPlayers(int i)
        {
            List<SeenCoachObject> palyers = new List<SeenCoachObject>();
            var dic = GetSeenCoachObjects();
            var teamName = SoccerParams.m_teamName;
